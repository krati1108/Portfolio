...
const ProjectCard = ({ project, aosDelay }) => (
  ...
  >
    <div className={`rounded-2xl p-6 md:p-8 h-full backdrop-blur-md transition-all duration-500 ${
      ...
      <div className="relative w-full aspect-[16/9] overflow-hidden rounded-xl mb-8 bg-black border border-white/10">
        <img
          src={project.image}
      ...
      {/* Badge */}
      {project.badge && (
        <span className="inline-block text-xs font-bold tracking-widest uppercase text-sky-400 bg-sky-500/10 px-3 py-1 rounded-full border border-sky-500/20 mb-4">
          {project.badge}
        </span>
      )}
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {project.techTags.map((tag) => (
          <span 
            key={tag}
            className="px-3 py-1 text-xs font-bold text-white/70 bg-white/5 rounded-full border border-white/10 hover:bg-sky-500/20 hover:border-sky-500/30 hover:text-sky-300 transition-all duration-300 cursor-default"
          >
            {tag}
          </span>
        ))}
